...
export default function SearchField(props) {
  const { onSelect, onClick, placeholder, searchSimple, hideFilter, apiClient, handleSearch, ...rest } = props
    ...
    <InputFieldWrapper className={styles.rawSearch}>
      {searchSimple?.isSearchSimple ? (
        <div className={styles.searchField}>
          <Input
            ...
            suffix={searchSimple?.value?.length === 0 && <SvgIcon name="search" className={styles.iconSearch} />}
        ...
        >
          <Input
            ...
            allowClear
            onChange={searchSimple?.onChange}
            suffix={<SvgIcon name="search" className={styles.iconSearch} onClick={handleSearch} />}
            {...rest}
          />
        </MyAutoComplete>
      )}
      ...
      {!hideFilter && (
        <Tooltip placement="bottom" mouseLeaveDelay={0} title="Tìm kiếm nâng cao">
          <Button btnType="greyFilter" shape="circle" iconName="filter-white" onClick={onClick} />
        </Tooltip>
      )}
    </InputFieldWrapper>
  )
