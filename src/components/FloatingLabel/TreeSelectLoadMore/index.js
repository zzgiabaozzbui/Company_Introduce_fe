...
export default function TreeSelectLoadMore(props) {
  ...
  useEffect(() => {
    const valueField = document.querySelector(`.h_select-tree_${randomClass} .ant-select-selection-item`)
    if (valueField) {
  ...
  if (totalNode) {
    if (selectedArray?.length === 0 || selectedArray?.length === totalNode) {
      tProps = { ...tProps, maxTagPlaceholder: OPTION_ALL_LABEL, maxTagCount: 0 }
    } else if (selectedArray.length > 10) {
      tProps = {
        ...tProps,
        maxTagPlaceholder: () => <span style={{ width: 200 }}>{`${selectedArray.length}/${totalNode} ${label}`}</span>,
        maxTagCount: 0
      }
    }
  }
  if (customTreeData.length > 0) tProps = { ...tProps, treeDefaultExpandedKeys: [`${customTreeData[0].key}`] }

  return (
    <TreeSelectWrapper
      bgcolor={bgcolor}
      isFocus={isFocus}
      isAll={!selectedArray || selectedArray.join() === '0' || selectedArray.join() === '-1'}
      className={`${className} ${styles.fieldWrapper} h_select-tree_${randomClass} floating-label`}
    >
      <FloatingLabel bgcolor={bgcolor} label={!hideLabel && label} isRequired={isRequired}>
        <TreeSelect
          {...treeProps}
